...
**Modelo de registro:** el registro público (`POST /api/auth/registro`, `RegistroService`) crea **siempre** un `CLIENTE`. El `RegistroRequest` exige nombre, email, contraseña (8–72) y **teléfono obligatorio de exactamente 10 dígitos** (`@Pattern("\\d{10}")`). El personal (BARISTA/CAJERO/SUPERVISOR/ADMIN) lo da de alta un **ADMIN** vía `POST /api/interno/trabajadores` (`TrabajadorAdminService`, restringido a `hasRole("ADMIN")` en `SecurityConfig`), eligiendo el rol explícitamente. Ya no existe la "clave mágica" de registro. Como el primer admin no puede crearse por API (no hay admin todavía), se siembra a mano con [db/seed_primer_admin.sql](db/seed_primer_admin.sql).

**Autoservicio del cliente:** el cliente autenticado (rol `CLIENTE`) gestiona su propia cuenta vía `ClienteController` / `ClienteService`: consulta su perfil (`GET /api/clientes/me`) y **actualiza sus datos** editables —nombre y teléfono, mismo patrón de validación de 10 dígitos— con `PUT /api/clientes/me`. La escritura sigue la misma restricción que el cambio de contraseña: como `ROL_CLIENTE` **no tiene SELECT sobre la tabla base `CLIENTES`**, el update va por una query `@Modifying` dedicada en `ClienteRepository` (no se carga la entidad) y necesita un grant `UPDATE` por columna en Oracle — ver [db/fix_grant_update_datos_cliente.sql](db/fix_grant_update_datos_cliente.sql) (análogo a [db/fix_grant_update_password_cliente.sql](db/fix_grant_update_password_cliente.sql)). El **historial de compras** es `GET /api/pedidos/historial` (`PedidoService.historialCompras`): igual que `GET /api/pedidos` pero filtrando los pedidos que quedaron en `PENDIENTE_PAGO`, es decir solo las compras que el cliente llegó a pagar.
...
- El CORS lo configura `SecurityConfig` desde `app.cors.allowed-origins` (variable `CORS_ALLOWED_ORIGINS`, lista separada por comas), con `http://localhost:4200` (el frontend Angular de desarrollo) como valor por defecto.
